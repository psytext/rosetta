...
#' remain silent.
#' @param overrideVarNames Can be used to override the variable names (most
#' useful in functions).
#' @param na.print How to print missing values.
#' @param \dots Any additional arguments are passed to the `print` or
#' `pander` function.
#' @param headerStyle The header pre- and suffix to use when pandering the
#' result (useful when working with Markdown).
...
oneway <- function(y, x, posthoc=NULL, means=FALSE, fullDescribe=FALSE,
  ...
  if (is.null(overrideVarNames)) {
    res$input$x.name <- ufs::extractVarName(deparse(substitute(x)));
    res$input$y.name <- ufs::extractVarName(deparse(substitute(y)));
  } else {
    res$input$x.name <- overrideVarNames['x'];
    res$input$y.name <- overrideVarNames['y'];
  }
  ...
  res$intermediate$aov <- stats::aov(stats::formula(paste0(res$input$y.name, " ~ ",
                                             res$input$x.name)));

  res$intermediate$Anova <- car::Anova(res$intermediate$aov, type=3);

  if (!is.null(posthoc)) {
    if (tolower(posthoc)=="tukey") {
      res$intermediate$posthoc <- stats::TukeyHSD(res$intermediate$aov);
    }
      ...
      res$intermediate$posthoc <-
        stats::pairwise.t.test(x=y, g=x, p.adjust.method=posthoc);
    }
  }
  if (means) {
    res$intermediate$means <- psych::describeBy(y, x);
  ...
  res$intermediate$etasq <- ufs::computeEffectSize_etasq(var1=x, var2=y,
                                                         conf.level=conf.level);

  res$intermediate$confIntOmegaSq <- ufs::confIntOmegaSq(var1=x, var2=y,
                                                         conf.level=conf.level);
  ...
  if (corrections) {
    res$intermediate$welch <- stats::oneway.test(stats::formula(paste0(res$input$y.name,
                                                                " ~ ",
                                                                res$input$x.name)));

    ### Based on Brown & Forsythe (1974), found through Field (2014)
    SSm <- res$output$dat['Between groups (error + effect)', 'SS'];
    tmpDat <- stats::na.omit(data.frame(x=x, y=y));
    groupVariances <- as.numeric(by(tmpDat$y, tmpDat$x, stats::var));
    ...
    res$intermediate$brown.forsythe$p <- stats::pf(res$intermediate$brown.forsythe$F,
...
pander.oneway <- function(x, digits=x$input$digits,
  ...
  if(x$input$t) {
    pander::pander(t(x$output$dat), missing="");
  } else {
    pander::pander(x$output$dat, missing="");
  }
  ...
  if (!is.null(x$input$posthoc)) {
    ...
    if (!is.null(x$output$posthocLetters)) {
      cat("\n\n");
      pander::pander(x$output$posthocLetters$Letters);
    }
  }
